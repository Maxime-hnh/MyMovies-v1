        ...
        fetch(`https://api.themoviedb.org/3/movie/${movieId}/credits?api_key=${apiKey}&language=fr-FRA`)
            ...
            .then(creditsData => {
                ...
                limitedActors.forEach(actorData => {
                    ...
                    profilPathElement.addEventListener('click', () => {
                        const actorId = actorData.id;
                        window.location.replace(`./actor.html?id=${actorId}&movieId=${movieId}`);
                    })
                })
            })
    })
    ...
    .then(imagesData => {
        ...
        imageElements.forEach((imageElement, index) => {
            imageElement.addEventListener('click', () => {
                ...
                const imageSrc = imageElement.src.replace('/w500', '/original')
        ...
        previousButton.addEventListener('click', () => {
            ...
            const imageSrc = imageElements[currentImageIndex].src.replace('/w500', '/original')
            modalImage.src = imageSrc
        })
        ...
        recommendationMovies.forEach(recommendationMovie => {
            ...
            recommendation.addEventListener('click', (event) => {
                ...
                if (clickedElement) {
                    ...
                    window.location.href = `./dRecommandation.html?id=${movieId2}`;
                }
            })
        })
    })
...
window.addEventListener('beforeunload', function() {
    window.location.href += '?state=' + encodeURIComponent(JSON.stringify(window.history.state));
  });
